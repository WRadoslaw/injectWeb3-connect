...
export const toNamedInjectedWindowProvider = (
	rawExtension: [string, InjectedWindowProvider],
): NamedInjectedWindowProvider => ({
	name: rawExtension[0],
	version: rawExtension[1].version,
	enable: rawExtension[1].enable,
})

export const enableSingleExtension =
	(origin: string) =>
	(extension: NamedInjectedWindowProvider): Promise<InjectedExtension> =>
		extension.enable(origin).then((injected) => ({
			name: extension.name,
